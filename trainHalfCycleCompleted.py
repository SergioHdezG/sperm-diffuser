...
G_B2A_Dec = moduleHalfCycle.BackwardResidualGeneratorDecoder(latent_input_shape=G_B2B_Enc.outputs[0].shape[1:],
                                                            res1_input_shape=G_B2B_Enc.outputs[1].shape[1:],
                                                            res2_input_shape=G_B2B_Enc.outputs[2].shape[1:],
                                                            res3_input_shape=G_B2B_Enc.outputs[3].shape[1:],
                                                            output_channels=channels)
...
def G_B2A(B, mask, training=True):
    img = (((B+1)/2)*mask)*2 - 1.
    B_latent, B_res1, B_res2, B_res3 = G_B2B_Enc(img)
    B2A = G_B2A_Dec([B_latent, B_res1, B_res2, B_res3], training=training)
    return B2A, B_latent, B_res1, B_res2, B_res3
...
def train_G(A, B, A_mask, B_mask, A_rand, B_rand):
    with tf.GradientTape() as t:
        ...
        full_B2B_masked, full_B_latent, full_B_res1, full_B_res2, full_B_res3 = G_B2B_masked(B, B_mask, B_code_sample)
        ...
        B2A_B_mask, B_latent_B_mask, B_res1_B_mask, B_res2_B_mask, B_res3_B_mask = G_B2A(B, B_mask, training=True)
        B2A_A_mask = G_B2A_Dec([B_latent, B_res1, B_res2, B_res3], training=True)
        ...
        B2A_B_mask_d_logit = D_A(B2A_B_mask, training=True)
        B2A_A_mask_d_logit = D_A(B2A_A_mask, training=True)
        B2A_B_mask_g_loss = g_loss_fn(B2A_B_mask_d_logit)
        B2A_A_mask_g_loss = g_loss_fn(B2A_A_mask_d_logit)
        ...
        B2B_full_masked_rec_loss = tf.reduce_mean(tf.losses.mse((((B+1)/2)*B_mask)*2 - 1., full_B2B_masked))
        ...
        G_loss = B2A_loss + (A2B_g_loss + B2B_g_loss) + (A2B_rand_g_loss + B2B_rand_g_loss) + B2B_rec_loss + B2B_masked_rec_loss + B2B_full_masked_rec_loss + latent_distillation - logpz * args.normal_prob_loss_weight
    ...
    return (A2B,
            ...
            B2B, A2B_rand, B2B_rand,
                      ...
                      'B2B_rand_g_loss': B2B_rand_g_loss,
                       'B2A_B_mask_g_loss': B2A_B_mask_g_loss,
                       'B2A_A_mask_g_loss': B2A_A_mask_g_loss,
                       'B2A_B_mask_selfrec_loss': B2A_B_mask_selfrec_loss,
                       'B2A_A_mask_selfrec_loss': B2A_A_mask_selfrec_loss,
...
@tf.function
def train_D(A, B, A2B, B2A_A_mask, B2A_B_mask, B2B, A_rand, B_rand, A2B_rand, B2B_rand):
    with tf.GradientTape() as t:
        ...
        D_B_loss = (B_d_loss + A2B_d_loss) + (BB_d_loss + B2B_d_loss) + \
...
def train_step(A, B, A_mask, B_mask, A_rand, B_rand):
    ...
    B2A_A_mask = B2A_pool(B2A_A_mask)  # because of the communication between CPU and GPU
    ...
    for ep in tqdm.trange(args.epochs, desc='Epoch Loop'):
        ...
        for [A, B, A_mask, B_mask], [A_rand, B_rand] in zip(tqdm.tqdm(A_B_dataset, desc='Inner Epoch Loop', total=len_dataset), A_B_dataset_random):
            ...
            if G_optimizer.iterations.numpy() % 250 == 0:
                ...
                A2B, B2B, B2B_masked, B2A = sample(A, B, A_mask, B_mask)
