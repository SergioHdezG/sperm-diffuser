...
G_A2B_Enc = moduleHalfCycle.ForwardResidualGeneratorEncoder(input_shape=(args.crop_size, args.crop_size, channels), output_channels=channels)
G_B2B_Enc = moduleHalfCycle.ForwardResidualGeneratorEncoder(input_shape=(args.crop_size, args.crop_size, channels), output_channels=channels)
...
def G_B2B_masked(B, mask, code, res3, training=True):
    img = (((B+1)/2)*mask)*2 - 1.
    latent, res1, res2, _ = G_B2B_Enc(img)
    B2B = G_A2B_Dec([latent, code, res1, res2, res3], training=training)
    return B2B, latent, res1, res2
...
def train_G(A, B, A_mask, B_mask, A_rand, B_rand):
    with tf.GradientTape() as t:
        ...
        B2B_masked, B_latent, B_res1, B_res2 = G_B2B_masked(B, A_mask, B_code_sample, A_res3)
        ...
        latent_distillation = tf.reduce_mean(tf.losses.mse(A_latent, tf.stop_gradient(B_latent)) + \
                                tf.losses.mse(A_res1, tf.stop_gradient(B_res1)) + \
                                tf.losses.mse(A_res2, tf.stop_gradient(B_res2)))
...
def sample(A, B, A_mask, B_mask):
    ...
    B2B, B_code_sample, B_code_mean, B_code_logvar = G_B2B(B, A_latent, A_res1, A_res2, A_res3, training=True)
    B2B_masked, B_latent, _, _ = G_B2B_masked(B, A_mask, B_code_sample, A_res3)
