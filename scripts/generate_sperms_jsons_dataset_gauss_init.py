...
savebase = 'diffuser/datasets/synth_dataset'

moving_DDPM = 'diffusion/defaults_H16_T20/progressive_model'
slow_DDPM = 'diffusion/defaults_H16_T20/slow_model'
inmotile_DDPM = 'diffusion/defaults_H16_T20/inmotile_model'
...
for i in range(n_sequences):
    ...
    for k in range(len(observations)):
        if make_subfolders:
            path_seq = os.path.join(savebase, 'json', f'field_{i}', 'json_bezier_spline', f'field_{i}_{k}')
        else:
            path_seq = os.path.join(savebase, 'json', f'field_{i}_{k}')
