...
py.arg('--output_dir', default='BezierImage2SpermImageMaskedPaired6')
...
G_B2A_Dec = moduleModified.BackwardResidualGeneratorDecoder(latent_input_shape=G_B2A_Enc.outputs[0].shape[1:],
...
def G_B2A(B, training=True):
    ...
    B2A = G_B2A_Dec([B_latent, res1, res2, res3], training=training)
    return B2A, B_latent, B_code, res1, res2, res3

def G_A2A(A_latent, res1, res2, res3, training=True):
    A2A = G_B2A_Dec([A_latent, res1, res2, res3], training=training)
    return A2A

def G_B2B(B_latent, B_code, res1, res2, res3, training=True):
...
def train_G(A, B, A_mask, B_mask):
    with tf.GradientTape() as t:
        ...
        A2B2A, A2B2A_latent, A2B2A_code, _, _, _ = G_B2A(A2B, training=True)

        B2A2B, B2A2B_latent, _, _, _ = G_A2B(B2A, code, training=True)

        # Modification
        # Reparametrized identity calculation
        # A2A = G_A2A(A_latent, A_res1, A_res2, A_res3, training=True)
        # B2B = G_B2B(B_latent, code, B_res1, B_res2, B_res3, training=True)
        ...
        A_mask_loss = reconstruction_mask_loss(A*A_mask, B2A*A_mask)  # + reconstruction_mask_loss(A*A_mask, A2A*A_mask)
        B_mask_loss = reconstruction_mask_loss(B*A_mask, A2B*A_mask)  # + reconstruction_mask_loss(B*B_mask, B2B*B_mask)

        G_loss = (A2B_g_loss + B2A_g_loss) + (A2B2A_cycle_loss + B2A2B_cycle_loss) * args.cycle_loss_weight \
                 + 10 * A_mask_loss + B_mask_loss # + (A2A_id_loss + B2B_id_loss) * args.identity_loss_weight

        # Modification
        # Identity in splines latent spaces
        spline_latent_loss = tf.reduce_mean(
           tf.losses.mse(A_latent, tf.stop_gradient(A2B2A_latent)) + tf.losses.mse(tf.stop_gradient(B_latent), B2A2B_latent))
...
def train_D(A, B, A2B, B2A,  A_mask, B_mask):
    with tf.GradientTape() as t:
        ...
        D_loss = 0.05*(A_d_loss + B2A_d_loss) + (B_d_loss + A2B_d_loss) + (D_A_gp + D_B_gp) * args.gradient_penalty_weight
...
@tf.function
def sample(A, B):
    B2A, B_latent, B_code, B_res1, B_res2, B_res3 = G_B2A(B, training=False)
    rand_code = tf.random.normal(shape=(*B_code.shape[:-1], B_code.shape[-1] // 2))
    A2B, A_latent, A_res1, A_res2, A_res3 = G_A2B(A, rand_code, training=False)
    A2B2A, _, _, _, _, _ = G_B2A(A2B, training=False)
    code = split_mean_var(B_code)
    B2A2B, _, _, _, _ = G_A2B(B2A, code, training=False)
    A2A = G_A2A(A_latent, A_res1, A_res2, A_res3, training=False)
    B2B = G_B2B(B_latent, code, B_res1, B_res2, B_res3, training=False)
